...
export default async function Home({params}: {params: Promise<{locale: string}>}) {
  ...
  return (
    <div className="flex flex-col gap-16 lg:gap-20">
      {/* Hero Section */}
      <section className="relative glass-panel p-5 lg:p-24 flex flex-col items-center text-center min-h-[calc(100svh-3.5rem)] justify-center fantasy-border lg:border-x-0 lg:border-t-0">
        <div className="relative z-10 max-w-4xl lg:max-w-5xl">
          ...
          <div className="mt-10 flex flex-wrap justify-center gap-4">
            <Link href={`/${locale}/encounter-builder`} scroll={false}>
      ...
      <section className="grid gap-12">
        <div className="flex flex-col items-center text-center">
          <h2 className="text-2xl lg:text-4xl font-serif accent-gold uppercase tracking-widest break-words">{t("home.coreToolkit.title")}</h2>
          <p className="text-muted mt-4 max-w-lg text-lg italic">{t("home.coreToolkit.description")}</p>
        </div>

        <div className="grid gap-8 sm:grid-cols-3">
          <Link href={`/${locale}/monster-scaler`} scroll={false} className="group">
            <Card className="p-10 border-gold/10 hover:border-gold/40 transition-all flex flex-col h-full bg-card">
              <div className="h-16 w-16 border border-gold/30 flex items-center justify-center mb-8 group-hover:border-gold transition-all bg-background text-gold">
              ...
              <h3 className="font-serif text-xl lg:text-2xl group-hover:text-gold transition-colors uppercase tracking-tight text-foreground break-words">{t("home.coreToolkit.monsterScaler.title")}</h3>
          ...
          <Link href={`/${locale}/encounter-builder`} scroll={false} className="group">
            <Card className="p-10 border-silver/10 hover:border-silver/40 transition-all flex flex-col h-full bg-card">
              <div className="h-16 w-16 border border-silver/30 flex items-center justify-center mb-8 group-hover:border-silver transition-all bg-background text-silver">
              ...
              <h3 className="font-serif text-xl lg:text-2xl group-hover:text-silver transition-colors uppercase tracking-tight text-foreground break-words">{t("home.coreToolkit.encounterBuilder.title")}</h3>
              <p className="text-muted mt-6 leading-relaxed flex-grow text-base">
          ...
          <Link href={`/${locale}/artifact-forge`} scroll={false} className="group">
            <Card className="p-10 border-gold/10 hover:border-gold/40 transition-all flex flex-col h-full bg-card">
              <div className="h-16 w-16 border border-gold/30 flex items-center justify-center mb-8 group-hover:border-gold transition-all bg-background text-gold">
              ...
              <h3 className="font-serif text-xl lg:text-2xl group-hover:text-gold transition-colors uppercase tracking-tight text-foreground break-words">{t("home.coreToolkit.artifactForge.title")}</h3>
              <p className="text-muted mt-6 leading-relaxed flex-grow text-base">
      ...
      <Card className="p-12 bg-card fantasy-border">
        <div className="grid gap-16 grid-cols-1 lg:grid-cols-2 items-center">
          ...
          <div className="relative aspect-video border border-gold/20 bg-background flex items-center justify-center">
             <span className="text-gold/40 font-serif italic text-sm tracking-widest uppercase">{t("home.reliability.preview")}</span>
             {/* Abstract UI representation */}
             <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-4/5 h-3/4 border border-gold/30 bg-card p-6 flex flex-col gap-3">
                <div className="h-5 w-2/3 bg-gold/20 border-b border-gold/30" />
                <div className="h-2 w-full bg-foreground/5" />
                <div className="h-2 w-full bg-foreground/5" />
                <div className="grid grid-cols-3 gap-3 mt-6">
                  <div className="h-12 border border-gold/10 bg-gold/5" />
                  <div className="h-12 border border-gold/10 bg-gold/5" />
                  <div className="h-12 border border-gold/10 bg-gold/5" />
                </div>
             </div>
